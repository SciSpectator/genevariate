...
class PlotExportManager:
    ...
    def export_batch(self, figures: Mapping[str, object],
        ...
        """Save every figure in ``figures`` (key → Figure).

        Returns key→Path for the figures that were actually written. A figure
        that fails to save is skipped rather than recorded under a fabricated
        ``Path("ERROR: …")`` - that placeholder made callers count failures as
        saves and made the HTML index link to a file that does not exist.
        """
        if subdir is None:
        ...
        for key, fig in figures.items():
            ...
            try:
                ...
                continue  # one bad figure must not abort the batch
        return paths
