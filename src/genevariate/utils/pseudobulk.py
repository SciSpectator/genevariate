...
def pseudobulk(
    ...
    *,
    ...
    # Carry the study each group's cells came from. In CELLxGENE that is
    # dataset_id, the Census analogue of a GEO series. It is the unit of
    # replication for every study-aware statistic downstream, and the default
    # groupby (donor_id, cell_type) drops it -- so it is forwarded whether or
    # not it was grouped on, rather than left to the caller to remember.
    for src in ("dataset_id", "series_id", "gse"):
        if src in obs_out.columns:
            break
        if src in adata.obs.columns:
            majority = (adata.obs.assign(_g=group_key.values)
                        ...
                        .agg(lambda s: s.astype(str).mode().iloc[0]
                             if len(s.mode()) else ""))
            obs_out[src] = [majority.get(c, "") for c in cat_strs]
            break
...
# ────────────────────────────────────────────────────────────────────────────
# Scale harmonisation with the rest of the program
# ────────────────────────────────────────────────────────────────────────────
def is_raw_counts(X) -> bool:
    """True when a matrix holds raw read counts rather than normalized values.

    Non-negative and whole-valued is the whole test. The bulk detector
    (``gpl_downloader._looks_like_counts``) also demands a maximum above 1000,
    which guards against a heavily rounded array being read as a count table;
    that guard does not transfer here, because a gene-restricted single-cell
    fetch can be entirely counts and never reach 1000. Per-cell normalized
    values (CP10K, log1p) are fractional and fail the integrality test, which
    is the distinction that actually matters for this path.
    """
    arr = np.asarray(X.data if hasattr(X, "data") else X, dtype=np.float64).ravel()
    arr = arr[np.isfinite(arr)]
    if arr.size == 0:
        return False
    return bool(arr.min() >= 0.0 and np.all(arr == np.floor(arr)))


def normalize_pseudobulk(adata_pb):
    """Put a raw-count aggregate on the scale the rest of GeneVariate reasons on.

    A pseudo-bulk group is a sequencing library, so it carries the same two
    artefacts a bulk library does: the depth of the cells that went into it,
    and the composition bias a few dominant transcripts impose on every other
    gene. Aggregation removes neither. This sends the aggregate through the
    identical TMM → CPM → log2(CPM+1) route the program already applies to bulk
    RNA-seq counts, estimated from this matrix alone - no other platform is
    read, so the result is a within-platform normalization exactly like the
    microarray and RNA-seq ones.

    Returns the input unchanged when it does not hold counts.
    """
    ad_mod = _require_anndata()
    X = adata_pb.X
    if hasattr(X, "toarray"):
    ...
    if X.size == 0 or not is_raw_counts(X):
        # Returning the input unchanged is the right call - there is nothing
        # safe to do with a matrix whose scale is unknown - but doing it
        # silently is not. A caller that believes this normalized registers
        # the platform as "log2 CPM (pseudobulk)", and every bound quoted
        # against that platform is then wrong by orders of magnitude. Record
        # the skip so the scale can be read off the object rather than
        # inferred from the promise in this docstring.
        info = dict(adata_pb.uns.get("pseudobulk", {}))
        info["normalization"] = "none (input is not raw counts)"
        info["normalization_skipped"] = True
        info["normalization_skip_reason"] = (
            "matrix is empty" if X.size == 0 else
            "values are not whole numbers - they are already normalized, or "
            "they are a mean of counts; aggregate raw counts with agg='sum' "
            "to produce a library this can normalize")
        adata_pb.uns["pseudobulk"] = info
        return adata_pb

    from genevariate.core.gpl_downloader import normalize_counts

    # Position, not gene name, indexes the frame: var_names can repeat, and the
    # kept positions are what slices var back into alignment with the matrix.
    frame = pd.DataFrame(X.T, index=np.arange(X.shape[1]),
                         columns=[str(s) for s in adata_pb.obs_names])
    norm, n_dropped = normalize_counts(frame)
    keep = norm.index.to_numpy()

    uns = dict(adata_pb.uns)
    info = dict(uns.get("pseudobulk", {}))
    info["normalization"] = "TMM -> CPM -> log2(CPM+1)"
    info["normalization_scope"] = "this platform only"
    info["genes_dropped_low_count"] = int(n_dropped)
    uns["pseudobulk"] = info

    return ad_mod.AnnData(X=norm.to_numpy(dtype=np.float32).T,
                          obs=adata_pb.obs.copy(),
                          var=adata_pb.var.iloc[keep].copy(),
                          uns=uns)
...
def pseudobulk_to_platform_df(adata_pb) -> pd.DataFrame:
    ...
    Output columns:
        * ``GSM`` - the pseudo-bulk group id, and ``series_id`` when the cells
          carried a study (``dataset_id`` / ``gse``)
        * obs columns (groupby fields, ``n_cells``, ``pseudobulk_method``,
          Classified_* aliases, …)
        * gene columns (from ``adata_pb.var_names``)

    GeneVariate's canonical frame keys samples by ``GSM`` and groups them into
    independent studies by ``series_id``. A pseudo-bulk group is a sample in
    exactly that sense - an aggregate of real measurements - so it is given
    both here. Without them the analysis suite rejects the frame outright, and
    every study-aware test that does accept it silently falls back to pooling
    groups that share a dataset as though they were independent evidence.

    No study column is invented when the cells carried none: one dataset is one
    study, and ``None`` from the study map is the honest answer.
    """
    _require_anndata()
    X = adata_pb.X
    if hasattr(X, "toarray"):
    ...
    if "series_id" not in meta.columns:
        for src in ("dataset_id", "gse"):
            if src in meta.columns:
                meta.insert(0, "series_id", meta[src].astype(str))
                break
    if "GSM" not in meta.columns:
        meta.insert(0, "GSM", [str(i) for i in adata_pb.obs.index])
    # Order: metadata first (so LabelEnrichmentWindow finds Classified_* early),
    # gene columns after.
    return pd.concat([meta, expr], axis=1, copy=False)


#: Fields a cell-level frame may use for the donor and the cell type, in the
#: order they are preferred. A pseudo-bulk group is a donor's cells of one
#: type, because cells are not independent replicates and the donor is the
#: level at which an aggregate is a sample at all.
DONOR_FIELDS = ("donor_id", "sample_id", "patient_id", "patient", "batch")
CELLTYPE_FIELDS = ("cell_type", "celltype", "CellType", "cell_ontology_class",
                   "cluster", "leiden", "louvain")
#: Groups thinner than this are dropped: a two-cell "sample" is noise wearing
#: a sample's name.
MIN_CELLS = 10


def choose_groupby(adata) -> list:
    """Donor x cell type for *adata*, falling back to what it does carry."""
    obs = [str(c) for c in adata.obs.columns]
    donor = next((c for c in DONOR_FIELDS if c in obs), None)
    ctype = next((c for c in CELLTYPE_FIELDS if c in obs), None)
    picked = [c for c in (donor, ctype) if c]
    if picked:
        return picked
    picked = [c for c in ("tissue", "cell_type") if c in obs]
    return picked or list(adata.obs.columns[:1])


def aggregate_to_platform(adata, *, groupby=None, agg=None,
                          min_cells: int = MIN_CELLS):
    """Cells to a registrable platform frame: the program's one way to do it.

    Every route into the program that turns cells into a platform - the
    browser window's "Load as platform", the assistant's Census fetch, a local
    ``.h5ad`` - has to make the same three choices, and when they were made
    separately they were made differently: one summed and one averaged, one
    grouped by donor and one by tissue, one kept groups of five cells and one
    of ten. The three answers are not interchangeable. A mean of raw counts is
    neither counts nor normalized, so the normalization below silently skips
    it and the platform lands on a raw scale that no bound quoted against a
    log2 platform can be compared with; grouping by tissue collapses every
    donor of a cell type into one profile; and the looser cell threshold keeps
    exactly the aggregates least able to support a statistic.

    Returns ``(frame, pb)`` - the platform DataFrame and the pseudo-bulk
    AnnData behind it, whose ``uns['pseudobulk']`` records what was done.
    """
    if groupby is None:
        groupby = choose_groupby(adata)
    if agg is None:
        # Summed when the input holds counts: a group of cells is a library
        # only once they are added up, and the bulk normalization acts on
        # libraries. Already-normalized input is averaged.
        agg = "sum" if is_raw_counts(adata.X) else "mean"
    pb = pseudobulk(adata, groupby=tuple(groupby), min_cells=int(min_cells),
                    agg=agg)
    pb = normalize_pseudobulk(pb)
    frame = pseudobulk_to_platform_df(pb)
    if "GSM" not in frame.columns:
        frame = frame.reset_index()
        frame = frame.rename(columns={frame.columns[0]: "GSM"})
        frame["GSM"] = frame["GSM"].astype(str)
    return frame, pb
