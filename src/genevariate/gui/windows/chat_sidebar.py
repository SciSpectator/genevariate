...
"thinking" strip. When the reasoning loop is unavailable it asks the model for a
single tool instead, and when no model can be reached it says so rather than
guessing. Full result tables/reports open in a separate results window; the
chatbox keeps only a short summary line.
...
try:
    from genevariate.utils.viz_style import AERO, make_interactive as _make_interactive
...
# The model answers in markdown and writes its numbers in TeX. A Tk Text
# widget renders neither, so the reader was shown "**GAPDH**" and
# "$1.24 \times 10^{-23}$" verbatim. The maths is turned into real
# characters and the emphasis into text tags, because an exponent typed as
# ^{-23} is not a number a reader can check at a glance.
...
_TEX_WORDS = {
    ...
    r"\rightarrow": "\u2192", r"\to": "\u2192", r"\alpha": "\u03b1",
    r"\beta": "\u03b2", r"\mu": "\u00b5", r"\sigma": "\u03c3",
    r"\chi": "\u03c7", r"\Delta": "\u0394", r"\percent": "%",
    r"\,": " ", r"\;": " ", r"\!": "", r"\left": "", r"\right": "",
}


def _tex_to_unicode(expr: str) -> str:
    """One TeX fragment as characters a Tk font can draw."""
    out = expr
    for k, v in sorted(_TEX_WORDS.items(), key=lambda kv: -len(kv[0])):
        tail = r"(?![A-Za-z])" if k[-1].isalpha() else ""
        out = re.sub(re.escape(k) + tail, v.replace("\\", "\\\\"), out)
    out = re.sub(r"\^\{([^{}]*)\}", lambda m: m.group(1).translate(_SUP), out)
    out = re.sub(r"\^(-?\w)", lambda m: m.group(1).translate(_SUP), out)
    out = re.sub(r"_\{([^{}]*)\}", lambda m: m.group(1).translate(_SUB), out)
    out = re.sub(r"_(\w)", lambda m: m.group(1).translate(_SUB), out)
    out = re.sub(r"\\mathrm\{([^{}]*)\}", r"\1", out)
    out = re.sub(r"\\text\{([^{}]*)\}", r"\1", out)
    return out.replace("{", "").replace("}", "")


def _demath(text: str) -> str:
    r"""Replace every ``$...$`` / ``\(...\)`` span with real characters."""
    text = re.sub(r"\$\$(.+?)\$\$", lambda m: _tex_to_unicode(m.group(1)),
                  text, flags=re.S)
    text = re.sub(r"\$([^$\n]+?)\$", lambda m: _tex_to_unicode(m.group(1)), text)
    text = re.sub(r"\\\((.+?)\\\)", lambda m: _tex_to_unicode(m.group(1)),
                  text, flags=re.S)
    return text
...
class ChatSidebar(ttk.Frame):
    ...
    def __init__(self, master, app, **kwargs):
        ...
        # This panel's conversation. The agent replays the turns filed under it,
        # so "now split that gene by it" refers to the answer above rather than
        # starting from nothing.
        self._thread_id = f"chat-{uuid.uuid4().hex[:12]}"
        self._build()
        self._greet()
    ...
    def _build(self) -> None:
        ...
        self._transcript.tag_configure("report", foreground=D["muted"],
                                       font=("Consolas", 8))
        # Markdown emphasis, applied on top of the speaker tag so the colour
        # still says who is talking.
        self._transcript.tag_configure("md_bold", font=("Segoe UI", 10, "bold"))
        self._transcript.tag_configure("md_em", font=("Segoe UI", 10, "italic"))
        self._transcript.tag_configure("md_code", font=("Consolas", 9),
                                       foreground=D["tool"])
        self._transcript.tag_configure("md_h", font=("Segoe UI", 11, "bold"),
                                       foreground=D["accent_dark"]
                                       if "accent_dark" in D else D["user"],
    ...
    def _append(self, who: str, text: str, tag: str) -> None:
        ...
        if prefix:
            self._transcript.insert(tk.END, prefix, tag)
        if tag in ("bot", "final"):
            self._insert_markdown(str(text), tag)
        else:
            self._transcript.insert(tk.END, str(text), tag)
        self._transcript.insert(tk.END, "\n\n", tag)
        self._transcript.configure(state=tk.DISABLED)
        self._transcript.see(tk.END)

    #: ``**bold**``, ``` `code` ``` and ``*italic*`` inside one line.
    _MD_SPAN = re.compile(r"(\*\*.+?\*\*|`[^`]+`|(?<![\w*])\*[^*\n]+\*(?![\w*]))")

    def _insert_markdown(self, text: str, base_tag: str) -> None:
        """Write the model's markdown as formatted text, not as its source."""
        txt = self._transcript
        for raw_line in _demath(text).split("\n"):
            line = raw_line.rstrip()
            tag = base_tag
            head = re.match(r"^(#{1,6})\s+(.*)$", line)
            if head:
                line, tag = head.group(2), "md_h"
            elif re.match(r"^\s*([-*+]|\d+\.)\s+", line):
                indent = len(line) - len(line.lstrip())
                line = (" " * indent + "\u2022 "
                        + re.sub(r"^\s*([-*+]|\d+\.)\s+", "", line))
            elif set(line.strip()) <= {"-", "|", ":"} and line.strip():
                continue          # a markdown table rule draws nothing here
            for part in self._MD_SPAN.split(line):
                if not part:
                    continue
                if part.startswith("**") and part.endswith("**"):
                    txt.insert(tk.END, part[2:-2], (tag, "md_bold"))
                elif part.startswith("`") and part.endswith("`"):
                    txt.insert(tk.END, part[1:-1], (tag, "md_code"))
                elif (part.startswith("*") and part.endswith("*")
                      and len(part) > 2):
                    txt.insert(tk.END, part[1:-1], (tag, "md_em"))
                else:
    ...
    def _on_send(self) -> None:
        ...
        # is available, otherwise ask the model to pick a single tool.
    ...
    def _present_action(self, action, err) -> None:
        ...
        try:
            ...
            return
        self._append("bot",
                     f"I'll run “{tool.name}”. Review the parameters "
    ...
    def _on_run(self) -> None:
        ...
        def progress_cb(value: float, text: str) -> None:
            # A progress update is marshalled to the Tk thread, so the last one
            # can land after the bar has been released and put it back on
            # screen - reading as a run frozen at whatever step it named. The
            # flag drops those, and the release is queued behind them.
            if done["v"]:
                return
            try:
                self.app.after(0, lambda: (None if done["v"]
                                           else self.app.update_progress(value, text)))
            except Exception:
                pass
        ...
        def finish():
            ...
            if acquired:
                try:
                    self.app.after(0, self.app._release_progress)
                except Exception:
                    pass
            self._set_busy(False)
        ...
        def worker():
            try:
                result = tool.run(self.app, params, progress_cb)
    ...
    def _present_result(self, result) -> None:
        ...
        # A region result opens the Region Analysis window itself: the same
        # window, with the same tabs, that Analyze Selected Range opens. The
        # summary/table viewer is the fallback for everything else.
        if self._open_region_window(result):
            return
    ...
    def _open_region_window(self, result) -> bool:
        """Open Region Analysis for a result that describes a region.

        Returns False when the result is not a region one, or when the window
        cannot be built - the caller then falls back to the summary viewer, so
        a failure here costs the user a nicer window and not the answer.
        """
        try:
            spec_req = (getattr(result, "payload", None) or {}).get("region_window")
            if not spec_req:
                return False
            import pandas as pd
            from genevariate.gui.region_analysis import (
                RegionAnalysisWindow, build_region_spec,
            )
            plat = spec_req.get("platform")
            dfg = (getattr(self.app, "gpl_datasets", {}) or {}).get(plat)
            cuts = spec_req.get("cuts") or {}
            if dfg is None or not cuts:
                return False

            specs = []
            for col, (low, high) in cuts.items():
                if col not in dfg.columns:
                    continue
                spec = build_region_spec(
                    dfg, gene=col, platform=plat, column=col,
                    low=float(low), high=float(high),
                    label=f"{col}_R{len(specs) + 1}_{plat}",
                    log=getattr(self.app, "enqueue_log", None))
                if spec is not None:
                    specs.append(spec)
            if not specs:
                return False

            labels_df = getattr(self.app, "default_labels_df", None)
            if labels_df is None:
                per_plat = getattr(self.app, "platform_labels", {}) or {}
                if per_plat:
                    labels_df = pd.concat(list(per_plat.values()),
                                          ignore_index=True)
            RegionAnalysisWindow(
                parent=self.winfo_toplevel(), app_ref=self.app,
                regions_data=specs,
                mode="compare" if len(specs) > 1 else "analyze",
                platform_labels_df=labels_df)
            self._append("sys",
                         f"↑ Region Analysis opened for {len(specs)} region(s) "
                         f"on {plat}.", "sys")
            return True
        except Exception as exc:
            try:
                self._append("sys", f"(could not open Region Analysis: {exc})",
                             "sys")
            except Exception:
                pass
            return False
    ...
    def _open_results_window(self, result) -> None:
        ...
        if table is not None and hasattr(table, "empty") and not table.empty:
            ...
            # A tool can return a full-platform frame (thousands of rows / wide
            # gene matrices). ``to_string()`` on the whole thing builds a giant
            # string and stalls the Text widget - cap the rendered preview and
            # note the remainder (Open results / a dedicated export gives all).
            from genevariate.utils import display_limits
            _ROWS = display_limits.get('table_rows') or len(table)
            _COLS = display_limits.get('table_columns') or table.shape[1]
            try:
                shown = table.head(_ROWS)
                txt.insert(tk.END, shown.to_string(max_cols=_COLS) + "\n")
                extra = []
                if len(table) > _ROWS:
                    extra.append(f"{len(table) - _ROWS:,} more rows")
                if table.shape[1] > _COLS:
                    extra.append(f"{table.shape[1] - _COLS:,} more columns")
                if extra:
                    txt.insert(tk.END,
                               f"… ({', '.join(extra)}; preview capped at "
                               f"{_ROWS:,}×{_COLS})\n")
                txt.insert(tk.END, "\n")
            except Exception:
                txt.insert(tk.END, repr(table)[:20000] + "\n\n")
        report = (getattr(result, "report", "") or "").strip()
        if report:
    ...
    def _start_agent_worker(self, goal: str) -> None:
        ...
        def progress_cb(value: float, text: str) -> None:
            # See the note on the same guard in the single-tool path.
            if done["v"]:
                return
            try:
                self.app.after(0, lambda: (None if done["v"]
        ...
        def worker() -> None:
            try:
                ...
                if agent_available() and not self._stop_flag:
                    # A step budget the analyses actually need. The default of
                    # twelve is a sensible guard for a one-tool question, but a
                    # request that names several label columns, or three
                    # sources, spends a step per tool call and a step per model
                    # turn between them, and hits the limit mid-answer: the run
                    # then fails outright and the fallback planner reports
                    # "0/0 steps", so the user is left with nothing rather than
                    # with a partial result. Forty is the budget the case-study
                    # protocol was found to need, with headroom.
                    reply = run_agent(
                        self.app, reg, goal, on_event,
                        progress_cb=progress_cb, should_stop=stop,
                        thread_id=self._thread_id, max_steps=40)
                    used_reasoner = bool(getattr(reply, "ok", False))
                    if not used_reasoner:
                        on_event("sys",
                                 f"(reasoning model problem: {reply.summary}) - "
                                 "trying the planner instead.", None)
