...
def open_geometadb(db_path: str = None,
    ...
    if use_memory:
        ...
        try:
            ...
            disk_conn.close()
            conn = mem_conn
            # The copy only carries the indexes the source file already had.
            # A GEOmetadb straight from the download has none, so without this
            # every gsm/gse lookup in RAM mode was a full table scan - the one
            # mode that is supposed to be the fast one.
