...
class ToolResult:
    ...
    # Any further figures, keyed by a short name. Several windows answer one
    # question with more than one plot - a violin and the distance matrix it
    # summarises - and returning only the first left the rest unreachable and
    # unexportable. ``figure`` stays the one the results window embeds.
...
class Tool:
    ...
    def run(self, app, resolved: Dict[str, Any],
            progress_cb: Optional[Callable[[float, str], None]] = None) -> ToolResult:
        """Execute the tool and record what it produced.

        Callers should use this rather than reaching for ``executor``: an
        executor is handed only the app and its own parameters, so without a
        record of what has already been run there is no way to answer "save
        that plot" except by running the analysis a second time and hoping it
        comes back the same.
        """
        result = self.executor(app, resolved,
                               progress_cb or (lambda value, text: None))
        record(self.name, resolved, result)
        return result
...
@dataclass
class Recorded:
    """One completed tool run, kept so it can be exported or re-shown."""
    tool: str
    params: Dict[str, Any]
    result: ToolResult


# The figures and tables the assistant has produced this session, oldest first.
# Bounded, because a Figure holds its rendered canvas and an unbounded list of
# them is a slow leak in a window the user may leave open all day.
_HISTORY: List[Recorded] = []
_HISTORY_LIMIT = 40


def record(tool: str, params: Optional[Dict[str, Any]],
           result: ToolResult) -> None:
    _HISTORY.append(Recorded(tool, dict(params or {}), result))
    del _HISTORY[:-_HISTORY_LIMIT]


def history() -> List[Recorded]:
    """Every run so far, most recent last."""
