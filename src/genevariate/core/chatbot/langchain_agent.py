...
SYSTEM_PROMPT = (
    ...
    "   If a platform the user named is NOT already loaded, DECIDE ON YOUR OWN to "
    "obtain it: call `load_geo_platform` with that id — it AUTO-DOWNLOADS the "
    "platform straight from GEO when it isn't on disk. NEVER stop to ask the user "
    "to download data and NEVER tell them a file is missing; just download it and "
    "continue the task. Only report a data problem if the tool itself returns an "
    "error you cannot work around.\n"
