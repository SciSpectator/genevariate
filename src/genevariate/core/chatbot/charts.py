...
try:
    ...
    _C_MUTED = AERO.get("muted", "#5F7D95")
    _C_A_DARK = AERO.get("accent_dark", "#0A5B9A")
except Exception:  # pragma: no cover
    _C_A, _C_B, _C_LINE = "#1E90E0", "#2E7D32", "#0E2A45"
    _C_MUTED, _C_A_DARK = "#5F7D95", "#0A5B9A"


# ---------------------------------------------------------------- descriptors
from genevariate.core.analysis.figures import (  # noqa: E402
    describe_values, draw_gene_histogram,
)
...
def fig_histogram(values: Sequence[float], gene: str, label: str = "",
                  dist_class: str = "", x_label: str = "Expression",
                  bounds: Optional[Tuple[float, float]] = None,
                  ) -> Tuple[Any, Dict[str, Any]]:
    """The Explorer's histogram on a bare ``Figure``, for an executor thread.

    The drawing itself is not here: it is ``draw_gene_histogram``, the routine
    the Explorer window also calls. This wrapper exists only to own a Figure,
    because a tool has to return one and a window already has its own axis.
    An answer the assistant gives has to be an answer the user can reach by
    clicking, and the surest way to guarantee that is for both to run the same
    lines rather than two routines that were written to match.
    """
    ...
    desc = draw_gene_histogram(ax, values, gene=gene, label=label,
                               dist_class=dist_class, x_label=x_label,
                               bounds=bounds)
    # The bars and edges are a window's business; a returned figure does not
    # carry them into the transcript.
    desc.pop("bins", None); desc.pop("patches", None); desc.pop("modes", None)
    fig.tight_layout()
    return fig, desc
...
def fig_group_violin(groups: Mapping[str, Sequence[float]], title: str,
                     value_label: str = "expression", top: int = 12,
                     ) -> Tuple[Any, Dict[str, Any]]:
    """One violin per group, largest first - the Compare Distributions window's
    Separation tab.

    A distance matrix says how far apart two groups are but not *why*: a pair
    can be far apart because one sits higher, or because one is bimodal and
    straddles the other. The violins show which, and the box inside each one
    keeps the median and quartiles readable when the kernel is wide.
    """
    usable = []
    for name, values in groups.items():
        ...
        v = v[np.isfinite(v)]
        if v.size >= 2:
            usable.append((str(name), v))
    usable.sort(key=lambda p: p[1].size, reverse=True)
    hidden = max(0, len(usable) - top)
    usable = usable[:top]
    desc: Dict[str, Any] = {
        "title": title, "n_groups": len(usable), "hidden": hidden,
        "groups": [{"group": n, "n": int(v.size), "median": float(np.median(v)),
                    "iqr": float(np.subtract(*np.percentile(v, [75, 25])))}
                   for n, v in usable]}
    if not _HAS_MPL or not usable:
        return None, desc

    fig = Figure(figsize=(max(6.4, 0.9 * len(usable) + 2.0), 4.2), dpi=100)
    ax = fig.add_subplot(111)
    data = [v for _, v in usable]
    pos = np.arange(1, len(data) + 1)
    parts = ax.violinplot(data, positions=pos, showextrema=False, widths=0.85)
    for i, body in enumerate(parts["bodies"]):
        body.set_facecolor(_C_A if i % 2 == 0 else _C_B)
        body.set_edgecolor(_C_LINE)
        body.set_alpha(0.45)
        body.set_linewidth(0.6)
    ax.boxplot(data, positions=pos, widths=0.10, showfliers=False,
               medianprops=dict(color=_C_LINE, lw=1.4),
               boxprops=dict(color=_C_LINE, lw=0.9),
               whiskerprops=dict(color=_C_LINE, lw=0.9),
               capprops=dict(color=_C_LINE, lw=0.9))
    ax.set_xticks(pos)
    ax.set_xticklabels(
        [(n if len(n) <= 18 else n[:15] + "…") + f"\nn={v.size:,}"
         for n, v in usable], fontsize=7, rotation=30, ha="right")
...
def fig_distance_heatmap(matrix: Any, title: str,
                         ) -> Tuple[Any, Dict[str, Any]]:
    """Annotated square heatmap of a pairwise-distance matrix.

    Takes the matrix :func:`analysis.distribution_compare.distance_matrix`
    built from the pair table, so the picture and the numbers quoted beside it
    cannot drift apart.
    """
    names = [str(x) for x in matrix.index]
    m = np.asarray(matrix.to_numpy(), dtype=float)
    off = m[~np.eye(len(names), dtype=bool)] if len(names) > 1 else np.array([])
    desc: Dict[str, Any] = {"title": title, "n_groups": len(names),
                            "max": float(np.nanmax(off)) if off.size else None,
                            "min": float(np.nanmin(off)) if off.size else None}
    if off.size:
        i, j = np.unravel_index(np.nanargmax(np.where(
            np.eye(len(names), dtype=bool), -np.inf, m)), m.shape)
        desc["furthest"] = [names[i], names[j]]
    if not _HAS_MPL or not names:
        return None, desc

    side = max(4.0, 0.55 * len(names) + 2.2)
    fig = Figure(figsize=(side, side * 0.85), dpi=100)
    ax = fig.add_subplot(111)
    im = ax.imshow(m, cmap="Blues", aspect="auto")
    fig.colorbar(im, ax=ax, shrink=0.8)
    ax.set_xticks(range(len(names)))
    ax.set_yticks(range(len(names)))
    short = [n if len(n) <= 20 else n[:17] + "…" for n in names]
    ax.set_xticklabels(short, rotation=40, ha="right", fontsize=7)
    ax.set_yticklabels(short, fontsize=7)
    # Annotate every cell: a heatmap without the numbers can only be ranked by
    # eye, and the whole point of the matrix is the size of the gaps.
    hi = np.nanmax(m) if np.isfinite(m).any() else 1.0
    for i in range(len(names)):
        for j in range(len(names)):
            if np.isfinite(m[i, j]):
                ax.text(j, i, f"{m[i, j]:.2f}", ha="center", va="center",
                        fontsize=6,
                        color="white" if m[i, j] > hi * 0.6 else _C_LINE)
    ax.set_title(title, fontsize=11, weight="bold")
    fig.tight_layout()
    return fig, desc


def describe_group_violin_block(desc: Mapping[str, Any]) -> str:
    rows = desc.get("groups") or []
    if not rows:
        return ""
    lines = ["\n## What the chart shows"]
    top = max(rows, key=lambda r: r["median"])
    bot = min(rows, key=lambda r: r["median"])
    lines.append(f"- **Highest**: {top['group']} (median {top['median']:.3g}); "
                 f"**lowest**: {bot['group']} (median {bot['median']:.3g}).")
    widest = max(rows, key=lambda r: r["iqr"])
    lines.append(f"- **Widest**: {widest['group']} spans an IQR of "
                 f"{widest['iqr']:.3g} - a wide violin means the group is not "
                 f"one population, and its mean is not a summary of it.")
    if desc.get("hidden"):
        lines.append(f"- {desc['hidden']} smaller group(s) are not drawn.")
    return "\n".join(lines) + "\n"
