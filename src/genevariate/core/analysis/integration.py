...
def harmony_embed(sources: Dict[str, pd.DataFrame], n_pcs: int = 30
    ...
    matrix, batch = common_gene_matrix(sources)
    # Complete case, not zero-filled. A gene missing from some samples is
    # dropped here rather than set to zero: zero is a real expression level,
    # and a gene whose absence follows the source it came from would hand
    # Harmony a batch signal built out of the substitution itself, which is
    # the one thing an integration step must not manufacture.
    matrix = matrix.dropna(axis=0, how="any")
    X = matrix.T.values                                 # samples x genes
    ...
    # harmonypy's default nclust = round(n/30) collapses to 1 on small cohorts,
    # where the algorithm has nothing to soft-cluster and cannot mix batches.
    nclust = int(min(max(2, round(X.shape[0] / 30)), 100, max(X.shape[0] - 1, 2)))
    ho = harmonypy.run_harmony(pcs, meta, ["batch"], nclust=nclust)
    # harmonypy < 2 returns Z_corr as (PCs x samples); harmonypy >= 2 returns
    # (samples x PCs). Orient by the sample count rather than by version.
    corrected = np.asarray(ho.Z_corr)
    if corrected.shape[0] != X.shape[0]:
        corrected = corrected.T                         # -> samples x PCs
