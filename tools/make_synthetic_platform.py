...
def truth_markdown(expr_df, labels_df, blood_studies) -> str:
    ...
    lines = [
        ...
        "- Cross-fitted AUC for Liver must clear 0.80, which is the bound the "
        "validator enforces; folds are split by study, so the figure is held "
        "down by the label's clumping and lands near 0.82. The calibration "
        "error must not get worse after the isotonic layer.",
